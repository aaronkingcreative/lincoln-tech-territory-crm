  ...
  const db = createServiceClient(); const summary = empty();
    for (const [index, item] of items.entries()) {
      ...
      if (item.type === 'school_update') {
        ...
        if (school) { summary.updated.push({ ...base, record_id: school.id, school_record_id: school.id, reason: 'A safe existing-school match was found; this item will update it instead of creating a duplicate.', fields_changed: previewFields(item, school, schoolFields) }); continue; }
        const missing = requiredSchoolCreateMissing(item); const duplicate = missing.length ? null : await likelyDuplicateSchool(db, item); const districtResult = missing.length || duplicate ? null : await resolveDistrict(db, item, false);
      ...
      else if (item.type === 'task_create') summary.created.push({ ...base, fields_changed: [{ field:'title', label:'Title', to:item.title }, { field:'priority', label:'Priority', to:item.priority }, { field:'status', label:'Status', to:item.status }, { field:'notes', label:'Notes', to:item.notes ?? item.description }] });
