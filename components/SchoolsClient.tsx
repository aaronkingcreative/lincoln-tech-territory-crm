...
type Row = { s: School; miss?: unknown; latestNote?: unknown };
...
function notePreview(row: Row): string {
  ...
  const primary = [text(row.latestNote), text(row.s.outreach_notes), text(row.s.verification_notes)].find(value => value && !generic.test(value));
  if (primary) return primary;
  const source = text(row.s.source_notes);
  return source || 'No notes yet';
}
function missingFields(row: Row): string[] { return Array.isArray(row.miss) ? row.miss.filter((item): item is string => typeof item === 'string') : []; }
...
export default function SchoolsClient({ rows }: { rows: Row[] }) {
  ...
  const filtered = useMemo(() => rows.filter(row => {
    ...
    if (sort === 'missing') return missingFields(b).length - missingFields(a).length;
