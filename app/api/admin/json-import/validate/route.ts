...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      const base = { type: item.type, target_name: str(item.school_name) ?? str(item.district_name) ?? str(item.contact_name) ?? str(item.title), school: str(school?.name) ?? str(item.school_name), district: str(district?.name) ?? str(item.district_name), source_url: item.source_url };
      ...
      if ((item.type.includes('school') || item.type === 'task_create') && str(item.school_name) && !school) summary.warnings.push({ ...base, reason: 'School not found. This item may fail unless it is a global task.' });
