...
import { isPlaceholderDistrictName, missingItems } from '@/lib/coverage';
...
export default async function SchoolDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const placeholderDistrict = isPlaceholderDistrictName(school.districts?.name);
  ...
  <section className="grid gap-3 md:grid-cols-3"><Field label="District"><div>{school.districts?.name}</div>{placeholderDistrict ? <div className="mt-2 rounded-lg border border-amber-500/50 bg-amber-500/15 p-2 font-semibold text-amber-100">District needs verification</div> : null}</Field><Field label="Address">{[school.address, school.city, school.state, school.zip].filter(Boolean).join(', ')}</Field><Field label="Phone">{school.phone ? <a className="text-sky-300 underline" href={`tel:${String(school.phone).replace(/[^+\d]/g,'')}`}>{school.phone}</a> : null}</Field><Field label="Website">{school.website ? <a className="break-all text-sky-300 underline" href={school.website} target="_blank" rel="noreferrer">{school.website}</a> : null}</Field><Field label="Follow-up date">{school.next_follow_up_at}</Field><Field label="High School Last Visit">{formatDateOnly(school.last_high_school_visit_at)}</Field><Field label="Bell Schedule">{school.bell_schedule}</Field><Field label="Bell Schedule URL">{school.bell_schedule_url ? <a className="break-all text-sky-300 underline" href={school.bell_schedule_url} target="_blank" rel="noreferrer">Open bell schedule ↗</a> : null}</Field><Field label="Recruiting priority">{school.recruiting_priority}</Field><Field label="Relationship status">{school.relationship_status}</Field><Field label="School type">{school.school_type}</Field><Field label="Territory status">{school.territory_status}</Field><Field label="Missing data summary">{missing.join(', ') || 'None flagged'}</Field><Field label="Special programs">{school.special_programs}</Field><Field label="Program notes">{school.program_notes}</Field><Field label="Import notes">{school.outreach_notes}</Field><Field label="Source URL">{school.source_url ? <a className="text-sky-300 underline" href={school.source_url}>{school.source_url}</a> : null}</Field></section>
  <section className="grid gap-4 lg:grid-cols-2"><Panel title="Contacts" rows={contacts ?? []} fields={['name','title','email','phone','role_category','program_area','source_url','source_notes']} /><Panel title="Contact logs" rows={logs ?? []} fields={['contacted_at','contact_method','outcome','notes']} /><Panel title="Notes" rows={notes ?? []} fields={['created_at','note_type','note','created_by_email']} /><Panel title="Tasks" rows={tasks ?? []} fields={['title','status','priority','due_date','notes']} /><Panel title="Source URLs" rows={sources ?? []} fields={['url','page_title','http_status','last_crawled_at','notes']} /></section></main>;
}
