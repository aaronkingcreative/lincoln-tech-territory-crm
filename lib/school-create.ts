...
export const DISTRICT_VERIFICATION_NOTE = 'Created from field note. Location/district details need verification.';

export function placeholderDistrictName(county = '', state = '') {
  const normalizedCounty = county.trim().replace(/\s+county$/i, '');
  if (normalizedCounty && state.trim()) return `District To Verify - ${normalizedCounty} County, ${state.trim().toUpperCase()}`;
  if (state.trim()) return `District To Verify - Unknown County, ${state.trim().toUpperCase()}`;
  return 'District To Verify - Unknown Location';
}
...
export function requiredSchoolCreateMissing(item: JsonImportItem) {
  ...
  if (![item.city,item.county,item.state,item.address,item.phone,item.website,item.source_notes,item.note].some(str)) missing.push('one of city, county, state, address, phone, website, source_notes, or note');
  return missing;
}
...
export function rawSchoolCreatePayload(item: JsonImportItem, districtId: string, runId?: string) {
  ...
  const incomplete = placeholder || !str(item.county) || !str(item.city) || !str(item.state) || !str(item.address);
